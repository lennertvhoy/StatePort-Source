...
def plan(root: Path, action: str, inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "study-tip":
        completed = [item for item in activities if item.get("status") == "completed"]
        pending = [item for item in activities if item.get("status") != "completed"]
        if pending:
            tip = {
                "headline": "Continue your next planned activity",
                "detail": f"Start or resume {pending[0].get('label', 'your next activity')} while it is still fresh.",
            }
            focus = pending[0]
        elif completed:
            tip = {
                "headline": "Review what you finished",
                "detail": "Explain one completed activity back to yourself in your own words.",
            }
            focus = completed[-1]
        else:
            tip = {
                "headline": "Plan a first small activity",
                "detail": "Add one small planned activity so progress can start.",
            }
            focus = None
        return {
            "formatVersion": FORMAT,
            "actionId": "studystate.sample.study-tip/v1",
            "activity": focus,
            "tip": tip,
            "stateChangeProposals": [],
            "canonicalStateDigest": state_digest(root),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "redirect-activity", "undo-last-evidence", "study-tip",
        ),
    )
